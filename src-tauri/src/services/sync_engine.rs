...
impl SyncEngine {
    ...
    async fn execute_sync(
        ...
        task_repo
            .add_log(task_id, "info", &format!("Creating RisingWave CDC source for database {}...", request.mysql_database))
            .await?;
        ...
        task_repo
            .add_log(task_id, "info", &format!("Creating RisingWave table ods.{}...", request.mysql_table))
            .await?;
        ...
        sqlx::query(&create_db_ddl).execute(&sr_pool).await.map_err(|e| {
        ...
        sqlx::query(&sr_table_ddl).execute(&sr_pool).await.map_err(|e| {
            tracing::error!("Failed to create StarRocks table: {}", e);
            e
        })?;
        tracing::info!("Successfully created StarRocks table: {}.{}", request.target_database, request.target_table);
    ...
    async fn drop_risingwave_objects(
        ...
        tracing::info!("Note: Database-level source ods.ods_{} is retained for reuse", mysql_database);
