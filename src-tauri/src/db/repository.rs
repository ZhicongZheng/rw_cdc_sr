...
impl<'a> ConfigRepository<'a> {
    ...
    pub async fn update(&self, id: i64, req: CreateConnectionRequest) -> Result<()> {
        ...
        sqlx::query(
            ...
            SET name = ?, db_type = ?, host = ?, port = ?, username = ?, password = ?, database_name = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%SZ', 'now')
            WHERE id = ?
            "#,
        )
        .bind(&req.name)
    ...
    fn try_from(row: ConfigRow) -> Result<Self> {
        ...
        Ok(DatabaseConfig {
            ...
            created_at: DateTime::parse_from_rfc3339(&row.created_at)
                .map_err(|e| AppError::Config(format!("Invalid created_at: {}", e)))?
                .with_timezone(&Utc),
            updated_at: DateTime::parse_from_rfc3339(&row.updated_at)
                .map_err(|e| AppError::Config(format!("Invalid updated_at: {}", e)))?
                .with_timezone(&Utc),
        })
    }
}
